...
class LLMWebScraper(WebScraper):
    """Enhanced web scraper that uses a local LLM to analyze website content"""
    
    def __init__(self, 
                 ollama_model: str = "mistral:latest", 
                 cache_dir: str = "app/data/content_cache"):
        """Initialize LLM web scraper
            ...
            async with self.session.get(url, timeout=15) as response:
                ...
                try:
                    with open(cache_path, 'w', encoding='utf-8') as f:
                        json.dump({'url': url, 'content': text}, f, ensure_ascii=False)
                except Exception as e:
    ...
    async def analyze_with_llm(self, content: str, prompt: str) -> str:
        """Analyze content with local LLM using Ollama
        
        This function uses the Ollama CLI to analyze content with a local LLM.
        It creates a temporary file with the prompt and content, then calls
        the Ollama CLI to generate a response.
    ...
    async def analyze_url_relevance(self, url: str, disease_name: str) -> Tuple[bool, float, Dict[str, Any]]:
        """Analyze URL relevance to a disease using LLM
    ...
    async def extract_organization_info(self, url: str, disease_name: str) -> Optional[EnhancedSupportOrganization]:
        """Extract organization information from a website using LLM
    ...
    async def search_organizations(self, disease: DiseaseInfo, max_results: int = 10) -> List[EnhancedSupportOrganization]:
        """Search for organizations related to a disease using LLM for filtering
        ...
        # Analyze and filter URLs using LLM
        results = []
